...
//	private GameObject floor;
	...
	void Update () {
		...
		if (Input.GetButtonDown("Jump"))
		{
			...
			Camera.mainCamera.transform.position = new Vector3(MAP_WIDTH/2,100,MAP_HEIGHT/4);
...
//		floor.renderer.material.mainTexture = dungeonTexture;
		...
		Room playerRoom = rooms[r];
		player.SetActive(true);
		player.transform.position = new Vector3(playerRoom.boundary.center.x,1.0f,playerRoom.boundary.center.y);
...
//		GameObject.DestroyImmediate(floor);
		...
		enemy.transform.position = new Vector3(room.boundary.center.x,1.0f,room.boundary.center.y);

		// Find furthest room from player to place exit of dungeon
		Room maxRoom = rooms[0];
		for (int i = 0; i < rooms.Count; i++)
		{

			if(		Mathf.Sqrt( (rooms[i].boundary.center.x - playerRoom.boundary.center.x) 	* (rooms[i].boundary.center.x - playerRoom.boundary.center.x) 
			          		  + (rooms[i].boundary.center.y - playerRoom.boundary.center.y) 	* (rooms[i].boundary.center.y - playerRoom.boundary.center.y) 
			                ) 
			   > 	Mathf.Sqrt( (maxRoom.boundary.center.x - playerRoom.boundary.center.x) 	* (maxRoom.boundary.center.x - playerRoom.boundary.center.x) 
			        		  + (maxRoom.boundary.center.y - playerRoom.boundary.center.y) 	* (maxRoom.boundary.center.y - playerRoom.boundary.center.y)
			              	)
			) 
			{
